...
import { defaultRooms, defaultSections } from './defaultContent.js';
...
const app = express();
const PORT = Number(process.env.ADMIN_PANEL_PORT || 5051);
...
const db = new sqlite3.Database(DB_PATH, (error) => {
  if (error) {
    console.error('Error opening admin panel database:', error.message);
  } else {
    console.log(`Admin panel connected to SQLite database at ${DB_PATH}`);
  }
});
...
async function initializeDatabase() {
  ...
  for (const [key, value] of Object.entries(defaultSections)) {
    await run(
      `INSERT OR IGNORE INTO admin_sections (key, content_json) VALUES (?, ?)`,
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'success',
    message: 'Admin panel API running',
...
app.put('/api/admin/sections/:key', requireAuth, async (req, res) => {
  try {
    ...
    res.json({
      status: 'success',
      message: 'Seccion guardada correctamente.',
      data: merged
    });
  } catch (error) {
    res.status(500).json({ status: 'error', message: error.message });
...
app.post('/api/admin/rooms', requireAuth, async (req, res) => {
  try {
    ...
    if (existing) {
      res.status(409).json({
        status: 'error',
        message: 'Ya existe una sala con ese identificador.'
      });
      return;
    }
...
initializeDatabase()
  .then(() => {
    if (ADMIN_USER === 'admin' && ADMIN_PASSWORD === 'admin1234') {
      console.warn(
        'Admin panel is using default credentials. Change ADMIN_PANEL_USER and ADMIN_PANEL_PASSWORD before exposing it publicly.'
      );
    }
